...
🗑️  削除:
  npm run destroy:test: テスト環境削除（全スタック）
...
🧪 リグレッションテスト:
  ./scripts/regression-test.sh  : 自動リグレッションテスト実行
  
  テスト環境の推奨:
  1) test (推奨)  : リグレッションテスト専用環境
  2) dev          : 開発環境（他の作業に影響する可能性）
  3) staging      : ステージング環境
  4) prod         : 本番環境（非推奨）
